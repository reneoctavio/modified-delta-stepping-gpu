...
void delta_stepping_gpu_mpgu(CudaContext& context, HCsrMatrix *h_graph_light, HCsrMatrix *h_graph_heavy, const int k_delta, int ini_vertex) {
	// Vertices
	const int k_num_vertices = h_graph_light->num_rows;


	// Copy data to device
	MGPU_MEM(int) d_light_row_offsets = context.Malloc(h_graph_light->row_offsets.data(), k_num_vertices + 1);
	MGPU_MEM(int) d_light_column_indices;
	MGPU_MEM(int) d_light_values;
	if (h_graph_light->num_entries != 0)
	{
		d_light_column_indices = context.Malloc(h_graph_light->column_indices.data(), h_graph_light->num_entries);
		d_light_values = context.Malloc(h_graph_light->values.data(), h_graph_light->num_entries);
	}

	MGPU_MEM(int) d_heavy_row_offsets = context.Malloc(h_graph_heavy->row_offsets.data(), k_num_vertices + 1);
	MGPU_MEM(int) d_heavy_column_indices;
	MGPU_MEM(int) d_heavy_values;
	if (h_graph_heavy->num_entries != 0)
	{
		d_heavy_column_indices = context.Malloc(h_graph_heavy->column_indices.data(), h_graph_heavy->num_entries);
		d_heavy_values = context.Malloc(h_graph_heavy->values.data(), h_graph_heavy->num_entries);
	}
	...
	thrust::adjacent_difference(thrust::device, d_light_row_offsets->get() + 1, d_light_row_offsets->get() + k_num_vertices + 1, d_light_count->get());
	thrust::adjacent_difference(thrust::device, d_heavy_row_offsets->get() + 1, d_heavy_row_offsets->get() + k_num_vertices + 1, d_heavy_count->get());
	...
	while (true) {
		...
		while (true) {
    		...
    		thrust::for_each(thrust::device,
    				...
    				update_dist(d_distances->get(),
    						d_light_row_offsets.get()->get(),
    						d_light_column_indices->get(),
    						d_light_values->get(),
    						d_bucket_offset->get(),
    						d_vertex_id_bucket_dst->get(),
    						k_delta));
		...
		if (total != 0) {
			thrust::for_each(thrust::device,
					...
					update_dist(d_distances->get(),
							d_heavy_row_offsets.get()->get(),
							d_heavy_column_indices->get(),
							d_heavy_values->get(),
							d_deleted_offset->get(),
							d_vertex_id_bucket_dst->get(),
							k_delta));
		...
		d_vertex_id_bucket.swap(d_vertex_id_bucket_dst);
		d_deleted_offset = context.Fill(k_num_vertices, 0);
